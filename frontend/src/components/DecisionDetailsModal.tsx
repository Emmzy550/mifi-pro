        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
            <div className="bg-white rounded-2xl shadow-xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col font-sans">
                    ...
                    {activeTab === 'customer' && (
                        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div className="p-5 border border-slate-100 rounded-2xl bg-slate-50/50 transition-all hover:border-slate-200">
                                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1 font-mono">Recommended Limit</p>
                                    <p className="text-3xl font-bold text-slate-900 tracking-tight">${assessment.recommended_amount.toLocaleString()}</p>
                                </div>
                                <div className="p-5 border border-slate-100 rounded-2xl bg-slate-50/50 transition-all hover:border-slate-200">
                                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1 font-mono">Trust Score</p>
                                    <div className="flex items-center gap-3">
                                        {/* INVERTED LOGIC: High Risk = Low Trust */}
                                        <p className={`text-3xl font-bold tracking-tight ${assessment.risk_level === 'HIGH' ? 'text-red-600' : assessment.risk_level === 'MEDIUM' ? 'text-amber-600' : 'text-green-600'}`}>
                                            {assessment.risk_level === 'HIGH' ? 'LOW' : assessment.risk_level === 'MEDIUM' ? 'MEDIUM' : 'HIGH'}
                                        </p>
                                        <span className="text-xs text-slate-400">(Reflects stability & consistency)</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                <div className="px-6 py-3 border-t border-slate-200 bg-slate-50/50 flex justify-between items-center text-[10px] text-slate-400 font-mono tracking-tight">
                    ...
                    <span className="uppercase">Timestamp: {new Date().toISOString().split('T')[0]} • System Verified</span>
                </div>
            </div>
        </div>
    );
